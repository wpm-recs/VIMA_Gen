...
def get_code_reference_text() -> str:
    ...
    lines = [
        ...
        "# MANDATORY: 必须判空！空间不足时 get_random_pose 会返回 (None, None)，",
        "#            直接拿去做数值比较会抛 TypeError（'>' not supported ... NoneType）。",
        "if pose[0] is None or pose[1] is None:",
        "    raise ValueError('Failed to sample a free pose; retry or abort')  # 或重新采样",
        "",
        ...
        "",
        "# 获取 entry（R1：必须带 .value，否则拿到的是枚举成员，取 .size_range/.color_value 会报错）：",
        "obj_entry = ObjPedia.BOWL.value  # 或 ObjPedia.BLOCK.value 等",
        "texture_entry = TexturePedia.RED.value  # 或 TexturePedia.BLUE.value 等",
        "# R2：禁止调用 all_entries() / lookup_object_by_name() / lookup_color_by_name()",
        "#     —— 它们的类型标注说返回 entry，实际返回枚举成员。",
        "",
        "# 从 size_range 采样大小：",
        "size = self.get_random_size(obj_entry.size_range)",
        "",
        ...
        "========== check_success 典型实现 ==========",
        "",
        "# R4: ResultTuple 只有以下两个字段，绝对不要添加任何其它关键字参数：",
        "#     class ResultTuple(NamedTuple):",
        "#         success: bool",
        "#         failure: bool",
        "#     ResultTuple(..., distance=...) 会直接抛 TypeError。",
        "",
